...
class Index:
    ...
    def __init__(self, idx_list: Union[List, pd.Index, "Index", int]):
        ...
        if isinstance(idx_list, Index):
            ...
            # 修复 pandas 2.x 兼容性: pd.DatetimeIndex 迭代触发 C 层 SystemError
            # pd.Index 天然保证 dtype 一致，跳过逐元素类型检查
            if isinstance(idx_list, pd.Index):
                self.idx_list = idx_list.to_numpy()
                self.index_map = dict(zip(self.idx_list, range(len(self))))
                self._is_sorted = False
            else:
                first_type = type(idx_list[0])
                if not all(isinstance(x, first_type) for x in idx_list):
