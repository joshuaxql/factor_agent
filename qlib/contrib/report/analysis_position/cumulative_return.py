...
def cumulative_return_graph(
    ...
    """Backtest buy, sell, and holding cumulative return graph
        ...
        Example:
            ...
            .. code-block:: python
                ...
                features_df = D.features(D.instruments('csi500'), ['Ref($close, -6)/Ref($close, -1) - 1'], pred_df_dates.min(), pred_df_dates.max())
        ...
        Graph desc:
            ...
            - Axis X: Trading day.
            - Axis Y:
            - Above axis Y: `(((Ref($close, -6)/Ref($close, -1) - 1) * weight).sum() / weight.sum()).cumsum()`.
    ...
    :param label_data: `D.features` result; index is `pd.MultiIndex`, index name is [`instrument`, `datetime`]; columns names is [`label`].
        ...
        **The label T is the future 5-trading-day return from T+1 close to T+6 close**,
        example: `D.features(D.instruments('csi500'), ['Ref($close, -6)/Ref($close, -1)-1'])`
