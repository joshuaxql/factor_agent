...
class ProcessInf(Processor):
    ...
    def __init__(self, n_jobs=-1, resample_rule="ME"):
        self.n_jobs = n_jobs
    ...
    def __call__(self, df):
        def replace_inf(data):
            ...
            data = datetime_groupby_apply(data, process_inf, n_jobs=self.n_jobs, resample_rule=self.resample_rule)
...
class RobustZScoreNorm(Processor):
    ...
    def __call__(self, df):
        X = (df.loc[:, self.cols] - self.mean_train) / self.std_train
...
class CSRankNorm(Processor):
    ...
    def __call__(self, df):
        # try not modify original dataframe
        cols = get_group_columns(df, self.fields_group)
        dtype = df.loc[:, cols].dtypes.iloc[0]
