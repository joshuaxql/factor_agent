...
try:
    ...
    logger.warning("!!!!!!!! A error occurs when importing operators implemented based on Cython.!!!!!!!!")
    logger.warning("!!!!!!!! They will be disabled. Please Upgrade your numpy to enable them     !!!!!!!!")
...
class CSRank(ElemOperator):
    ...
    ``CSRank`` is calculated after all instruments have been assembled into a
    dataset. The per-instrument expression path only returns the underlying
    feature so the dataset provider can rank it by ``datetime``.
    """
