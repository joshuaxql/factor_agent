...
class DatasetProvider(abc.ABC):
    ...
    def dataset_processor(instruments_d, column_names, start_time, end_time, freq, inst_processors=[]):
        ...
        for field in column_names:
            ...
            if DatasetProvider._contains_cs_rank(expr):
                cross_fields.append(field)
            else:
        ...
        if normal_fields:
            frames.append(
                DatasetProvider._dataset_processor_no_cross(
                    instruments_d, normal_fields, start_time, end_time, freq, inst_processors=inst_processors
                )
            )
        if cross_fields:
            memo = {}
            cross_data = pd.concat(
                {
                    field: DatasetProvider._eval_cross_sectional_expression(
            ...
            frames.append(cross_data)

        if frames:
            data = pd.concat(frames, axis=1).sort_index()
            return data.reindex(columns=column_names)
        return pd.DataFrame(
    ...
    @staticmethod
    def _dataset_processor_no_cross(instruments_d, column_names, start_time, end_time, freq, inst_processors=[]):
        """
    ...
    @staticmethod
    def _contains_cs_rank(expr):
        from .base import Expression
        from .ops import CSRank, ChangeInstrument, ElemOperator, If, PairOperator, PairRolling, Rolling, TResample

        if isinstance(expr, CSRank):
            return True
        if isinstance(expr, (ElemOperator, Rolling, TResample)):
            return DatasetProvider._contains_cs_rank(expr.feature)
        if isinstance(expr, ChangeInstrument):
            return DatasetProvider._contains_cs_rank(expr.feature)
        if isinstance(expr, PairOperator):
            return (
                isinstance(expr.feature_left, Expression)
                and DatasetProvider._contains_cs_rank(expr.feature_left)
            ) or (
                isinstance(expr.feature_right, Expression)
                and DatasetProvider._contains_cs_rank(expr.feature_right)
            )
        if isinstance(expr, PairRolling):
            return (
                isinstance(expr.feature_left, Expression)
                and DatasetProvider._contains_cs_rank(expr.feature_left)
            ) or (
        ...
        if isinstance(expr, If):
            return any(
                isinstance(child, Expression) and DatasetProvider._contains_cs_rank(child)
                for child in (expr.condition, expr.feature_left, expr.feature_right)
            )
        return False

    @staticmethod
    def _eval_cross_sectional_expression(
        expr,
        instruments_d,
        start_time,
        end_time,
        ...
        from .base import Expression
        from .ops import CSRank, ChangeInstrument, ElemOperator, If, PairOperator, PairRolling, Ref, Rolling, TResample
        ...
        if not DatasetProvider._contains_cs_rank(expr):
            data = DatasetProvider._dataset_processor_no_cross(
                instruments_d, [key], start_time, end_time, freq, inst_processors=inst_processors
            )
            result = data[key]
            memo[key] = result
            return result
        ...
        if isinstance(expr, CSRank):
            value = DatasetProvider._eval_cross_sectional_expression(
                expr.feature, instruments_d, start_time, end_time, freq, inst_processors, memo
            )
            result = DatasetProvider._cs_rank(value)
        elif isinstance(expr, ChangeInstrument):
            raise NotImplementedError("CSRank inside ChangeInstrument is not supported.")
        elif isinstance(expr, TResample):
            raise NotImplementedError("CSRank inside TResample is not supported.")
        elif isinstance(expr, ElemOperator):
            value = DatasetProvider._eval_cross_sectional_expression(
                expr.feature, instruments_d, start_time, end_time, freq, inst_processors, memo
            )
            result = DatasetProvider._apply_numpy_func(value, expr.func)
        elif isinstance(expr, PairOperator):
            left = DatasetProvider._eval_cross_arg(
                expr.feature_left, instruments_d, start_time, end_time, freq, inst_processors, memo
            )
            right = DatasetProvider._eval_cross_arg(
                expr.feature_right, instruments_d, start_time, end_time, freq, inst_processors, memo
            )
            result = DatasetProvider._apply_numpy_func(left, expr.func, right)
        elif isinstance(expr, Ref):
            value = DatasetProvider._eval_cross_sectional_expression(
                expr.feature, instruments_d, start_time, end_time, freq, inst_processors, memo
            )
            result = DatasetProvider._ref_by_instrument(value, expr.N)
        elif isinstance(expr, Rolling):
            value = DatasetProvider._eval_cross_sectional_expression(
                expr.feature, instruments_d, start_time, end_time, freq, inst_processors, memo
            )
            result = DatasetProvider._rolling_by_instrument(value, expr.N, expr.func)
        elif isinstance(expr, PairRolling):
            left = DatasetProvider._eval_cross_arg(
                expr.feature_left, instruments_d, start_time, end_time, freq, inst_processors, memo
            )
            right = DatasetProvider._eval_cross_arg(
            ...
            result = DatasetProvider._pair_rolling_by_instrument(left, right, expr.N, expr.func)
        elif isinstance(expr, If):
            condition = DatasetProvider._eval_cross_arg(
                expr.condition, instruments_d, start_time, end_time, freq, inst_processors, memo
            )
            left = DatasetProvider._eval_cross_arg(
                expr.feature_left, instruments_d, start_time, end_time, freq, inst_processors, memo
            )
            ...
            index = DatasetProvider._first_series_index(condition, left, right)
            result = pd.Series(np.where(condition, left, right), index=index)
        else:
            raise NotImplementedError(f"Unsupported expression with CSRank: {expr}")
    ...
    @staticmethod
    def _eval_cross_arg(arg, instruments_d, start_time, end_time, freq, inst_processors, memo):
        from .base import Expression

        if isinstance(arg, Expression):
            return DatasetProvider._eval_cross_sectional_expression(
                arg, instruments_d, start_time, end_time, freq, inst_processors, memo
            )
        return arg

    @staticmethod
    def _cs_rank(series: pd.Series) -> pd.Series:
        def rank_one_day(day: pd.Series) -> pd.Series:
            count = day.notna().sum()
            if count == 0:
                return day
            return (day.rank(method="first") - 1) / count

        return series.groupby(level="datetime", group_keys=False).apply(rank_one_day)

    @staticmethod
    def _apply_numpy_func(left, func: str, right=None):
        if right is None:
            return getattr(np, func)(left)
        return getattr(np, func)(left, right)

    @staticmethod
    def _first_series_index(*values):
        for value in values:
    ...
    def _by_instrument(series: pd.Series):
        return series.sort_index(level=["instrument", "datetime"]).groupby(level="instrument", group_keys=False)

    @staticmethod
    def _ref_by_instrument(series: pd.Series, n: int) -> pd.Series:
        if n == 0:
            return DatasetProvider._by_instrument(series).transform(lambda inst: inst.iloc[0])
        return DatasetProvider._by_instrument(series).shift(n)

    @staticmethod
    def _rolling_by_instrument(series: pd.Series, n: int, func: str) -> pd.Series:
        def apply_one(inst: pd.Series) -> pd.Series:
            if func == "ref":
                return DatasetProvider._ref_by_instrument(inst, n)
            if func == "delta":
                return inst - inst.shift(n) if n != 0 else inst - inst.iloc[0]
            if func == "wma":
                return DatasetProvider._rolling_wma(inst, n)
            if func == "ema":
                return DatasetProvider._rolling_ema(inst, n)
            if func == "mad":
                return DatasetProvider._rolling_mad(inst, n)
            window = inst.expanding(min_periods=1) if n == 0 else inst.rolling(n, min_periods=1)
            if func == "rank":
                if hasattr(window, "rank"):
                    return window.rank(pct=True)
                return window.apply(lambda x: pd.Series(x).rank(pct=True).iloc[-1], raw=False)
            return getattr(window, func)()

        return DatasetProvider._by_instrument(series).apply(apply_one)

    @staticmethod
    def _rolling_mad(series: pd.Series, n: int) -> pd.Series:
        def mad(x):
            x1 = x[~np.isnan(x)]
            return np.mean(np.abs(x1 - x1.mean()))

        window = series.expanding(min_periods=1) if n == 0 else series.rolling(n, min_periods=1)
        return window.apply(mad, raw=True)

    @staticmethod
    def _rolling_wma(series: pd.Series, n: int) -> pd.Series:
        def weighted_mean(x):
            w = np.arange(len(x)) + 1
            w = w / w.sum()
        ...
        window = series.expanding(min_periods=1) if n == 0 else series.rolling(n, min_periods=1)
        return window.apply(weighted_mean, raw=True)

    @staticmethod
    def _rolling_ema(series: pd.Series, n: int) -> pd.Series:
        ...
        if n == 0:
            return series.expanding(min_periods=1).apply(exp_weighted_mean, raw=True)
        if 0 < n < 1:
            return series.ewm(alpha=n, min_periods=1).mean()
        return series.ewm(span=n, min_periods=1).mean()

    @staticmethod
    def _pair_rolling_by_instrument(left, right, n: int, func: str) -> pd.Series:
        index = DatasetProvider._first_series_index(left, right)
        df = pd.DataFrame({"left": left, "right": right}, index=index)

        def apply_one(inst: pd.DataFrame) -> pd.Series:
            window = inst["left"].expanding(min_periods=1) if n == 0 else inst["left"].rolling(n, min_periods=1)
            result = getattr(window, func)(inst["right"])
            if func == "corr" and n != 0:
                left_std = inst["left"].rolling(n, min_periods=1).std()
                right_std = inst["right"].rolling(n, min_periods=1).std()
                result.loc[np.isclose(left_std, 0, atol=2e-05) | np.isclose(right_std, 0, atol=2e-05)] = np.nan
            return result

        return df.sort_index(level=["instrument", "datetime"]).groupby(level="instrument", group_keys=False).apply(apply_one)
