...
cdef cnp.ndarray[cnp.float64_t, ndim=1] rolling(Rolling r, cnp.ndarray a):
    cdef int  i
    cdef int  N = len(a)
    cdef cnp.ndarray[cnp.float64_t, ndim=1] ret = np.empty(N)
    for i in range(N):
        ret[i] = r.update(a[i])
    return ret

def rolling_mean(cnp.ndarray a, int window):
    cdef Mean r = Mean(window)
    return rolling(r, a)

def rolling_slope(cnp.ndarray a, int window):
    cdef Slope r = Slope(window)
    return rolling(r, a)

def rolling_rsquare(cnp.ndarray a, int window):
    cdef Rsquare r = Rsquare(window)
    return rolling(r, a)

def rolling_resi(cnp.ndarray a, int window):
