    ...
    def get_quote_from_qlib(self) -> None:
        ...
        for attr in ("buy_price", "sell_price"):
            ...
            if self.quote_df[pstr].isna().any():
                self.logger.info("{} field data contains nan.".format(pstr))

        if (self.quote_df["$factor"].isna() & ~self.quote_df["$close"].isna()).any():
            self.trade_w_adj_price = True
            self.logger.info("factor.day.bin file not exists or factor contains `nan`. Order using adjusted_price.")
            if self.trade_unit is not None:
                self.logger.info(f"trade unit {self.trade_unit} is not supported in adjusted_price mode.")
        else:
